...
def assess(ct: "CorrelatedTelemetry") -> QualityAssessment:
    ...
    coverages.append(SignalCoverage(
    ...
    if not has_primary:
        findings.append(QualityFinding(
            signal="primary_trace_present", severity="critical",
...
def _rules_assessment(ct) -> tuple[list[str], list[str]]:
    ...
    has_state_transitions = any(
        signals.state_transitions(s) for s in ct.spans
    )
    has_retry_attempts = any(
        signals.retry_attempt(s) is not None for s in ct.spans
    )
